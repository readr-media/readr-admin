<template>
  <div class="item">
    ...
    <template v-if="!hasTitleSlot">
      <div class="id" v-text="get(member, 'id')"></div>
      ...
      <div class="status" v-text="$t(`member_page.STATUS.${get(STATUS, `${get(member, 'active')}`, 'NA')}`)"></div>
      <div class="register-type" v-text="$t(`member_page.REGISTER_MODE.${(get(member, 'registerMode', 'NA') || 'NA').toUpperCase()}`)"></div>
      <div class="actions">
        ...
        <div class="actions__update" v-if="$can('updateAccount')" v-text="$t('member_page.ADMIN_UPDATE')" @click="update(index)"></div>
        <div class="actions__delete" v-if="$can('deleteAccount')" v-text="$t('member_page.ADMIN_DELETE')" @click="del(index)"></div>
      </div>
    </template>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      canBeCustomEditor (m) {
        return get(this.roles, get(m, 'role', 1), '-') !== this.roles.MEMBER && this.get(this.roles, get(m, 'role', 1), '-') !== '-'
      },
      get,
    },
    mounted () {},
